...
現在は **PDF / PS / EPSの受付 → 永続ジョブ管理 → 隔離Ghostscript → TIFFファイル出力** を提供します。
...
対応するmanifestフィールドは `name`, `format`, `dpi`, `color_mode`, `priority` です。
...
- `rip-interpreter`: `DocumentInterpreter` 境界とGhostscriptアダプター。CoreはGhostscriptに依存しません。
- `jrip-server`: Axum HTTP API。32 MiB上限の原稿を逐次保存し、SHA-256を記録。
- `jrip-worker`: 別プロセスでジョブを処理。検証を通過したページ集合だけをディレクトリ単位で公開。

Ghostscriptはネットワークなし、read-only root、追加capabilityなし、no-new-privileges、CPU 1、メモリ／swap上限512 MiB、PID上限64、120秒のタイムアウトで動きます。
...
現在のCMYKはGhostscriptの標準処理です。独立LittleCMS層、ICC登録、黒版保持、CMYK16品質を提供するものではありません。
Ghostscriptの配布・製品化時は仕様書のライセンス方針に従って確認してください。このリポジトリには製品ライセンスを選定していません。
